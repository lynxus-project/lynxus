...
| First/second-level or session cache | Explicit rejection | Use an application cache outside Lynxus. The opt-in `QueryCache` adapter is a successful-SELECT cache, not a MyBatis session identity map. | [Design philosophy](../../Design-Philosophy.md#explicit-non-goals), [Extension contracts](extensions.md) |
| MyBatis plugin/interceptor chain | Explicit rejection | Use `ExecutionInterceptor` for observation or a typed whole-execution adapter for the documented plan replacement/short-circuit cases. These adapters cannot intercept JDBC phases, mutate generated binding/mapping, or provide full MyBatis plugin compatibility. | [Extension contracts](extensions.md), [Core contract](core-contract.md#3-jdbc-execution-contract) |
