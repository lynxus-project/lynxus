...
Parameter binders, providers, and row mappers are validated during compilation and directly referenced by generated code. Core standard routing uses generated Java types and JDBC metadata without scanning or reflective discovery. JDBC execution still belongs to `JdbcSqlExecutor`. Observational interceptors wrap `SqlExecutor` outside that lifecycle.
